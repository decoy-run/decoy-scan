...
export function classifyTool(tool) {
  ...
  // Substring fallback against name + desc — catches e.g. evaluate_script,
  // execute_query, where the anchored RISK_PATTERNS miss suffixed variants.
  const hay = `${name.toLowerCase()} ${desc}`;
  if (/execut|command|shell|bash|sudo|spawn|fork|eval/.test(hay)) return "critical";
  if (/delet|remov|drop|destruct|truncat|wipe|purge/.test(hay)) return "critical";
  if (/read.*file|write.*file|filesystem|file.*system/.test(hay)) return "high";
  if (/credentials|secret|password|api.?key|token|auth/.test(hay)) return "high";
  if (/http|request|fetch|curl|wget|network/.test(hay)) return "high";
  if (/database|query|sql|mongo|redis|dynamo/.test(hay)) return "high";
  if (/email|message|notification|sms|slack/.test(hay)) return "medium";
  if (/browse|screenshot|puppeteer|playwright|selenium/.test(hay)) return "medium";
...
export function analyzeServerCommand(entry) {
  ...
  // Supply-chain source integrity (rug-pull surface)
  findings.push(...analyzePackageSource(entry));
...
//
// A static scan can't see what a package *will* resolve to, but it can see
// whether the config lets it change at all. Two configurations re-fetch code on
// every launch with no review: a remote/VCS/URL source, and a package pinned to
// a moving dist-tag. Both are silent rug-pull surfaces. A concrete version pin
// (`pkg@1.2.3`) closes the door. Bare unpinned `npx pkg` is the documented norm
// for MCP servers, so it's deliberately *not* flagged here — only the
// genuinely-mutable sources are.

// Named dist-tags that float (re-resolve to whatever is published under them),
// as opposed to a concrete version or range.
const MUTABLE_DIST_TAGS = /^(?:latest|next|beta|alpha|canary|dev|nightly|edge|experimental|rc|unstable|insiders|main|master|head)$/i;

// Package runners that fetch + execute on launch. The first positional token
// after the runner (or the value of --package / --spec) is the package source.
const PACKAGE_RUNNERS = /\b(?:npx|bunx|uvx|npm\s+exec|pnpm\s+dlx|pnpm\s+exec|yarn\s+dlx|bun\s+x|pipx\s+run|deno\s+run)\b/i;

function classifyPackageSpec(spec) {
  if (!spec) return null;
  // Explicit remote/VCS/URL scheme — unreviewed code fetched from outside a
  // registry. (Bare `user/repo` shorthand is intentionally excluded: it's
  // ambiguous with relative paths like `dist/server.js`.)
  if (/^(?:github:|gitlab:|bitbucket:|gist:|git\+|git:\/\/|https?:\/\/|ssh:\/\/)/i.test(spec)) return "remote";
  // Local path — not a registry fetch, nothing to pin.
  if (/^(?:\.|\/|~|file:)/.test(spec)) return null;
  // Version is the last `@` that isn't the leading scope marker.
  const at = spec.lastIndexOf("@");
  if (at <= 0) return "unpinned";
  const version = spec.slice(at + 1);
  if (!version) return "unpinned";
  if (MUTABLE_DIST_TAGS.test(version)) return "mutable-tag";
  return "pinned";
}

export function analyzePackageSource(entry) {
  const cmd = entry.command || "";
  const argv = Array.isArray(entry.args) ? entry.args : [];
  const full = `${cmd} ${argv.join(" ")}`.trim();
  const findings = [];

  const runner = PACKAGE_RUNNERS.exec(full);
  if (!runner) return findings;

  // First positional token after the runner is the package source, unless an
  // explicit --package / --spec flag names it.
  const tokens = full.slice(runner.index + runner[0].length).trim().split(/\s+/).filter(Boolean);
  let spec = null;
  for (let i = 0; i < tokens.length; i++) {
    const tok = tokens[i];
    const eq = tok.match(/^(?:--package|--spec|--from)=(.+)$/i);
    if (eq) { spec = eq[1]; break; }
    if (/^(?:--package|--spec|--from|-p)$/i.test(tok)) { spec = tokens[i + 1]; break; }
    if (tok.startsWith("-")) continue; // other flags (-y, --yes, -q, …)
    spec = tok; break;
  }
  if (!spec) return findings;
  spec = spec.replace(/^['"]|['"]$/g, "");

  switch (classifyPackageSpec(spec)) {
    case "remote":
      findings.push({ type: "unpinned-remote-source", severity: "high",
        description: `Server runs code from a remote source "${spec}" instead of a pinned registry package — its contents can change between launches with no review` });
      break;
    case "mutable-tag":
      findings.push({ type: "unpinned-dist-tag", severity: "medium",
        description: `Server pins "${spec}" to a moving dist-tag — it re-resolves to whatever is published under that tag on every launch. Pin a concrete version (e.g. ${spec.slice(0, spec.lastIndexOf("@"))}@1.2.3)` });
      break;
  }
  return findings;
}
...
//
// env-exposure flags a *sensitive variable name*; this flags a literal secret
// *value* sitting in the config. A real token pasted into command/args/env
// leaks through dotfiles, backups, and version control — strictly worse than a
// `${VAR}` reference. The matched value is never echoed: findings carry only
// the credential type and its location, so scan output and telemetry can't leak
// the secret they just found.

// A value that's a reference or stand-in, not a real secret. Keeps the opaque
// heuristic from firing on `${GITHUB_TOKEN}`, `<your-key>`, `changeme`, etc.
function looksLikePlaceholder(v) {
  if (!v) return true;
  if (/^\$\{?[\w.-]+\}?$/.test(v)) return true;          // ${VAR} or $VAR
  if (/^%[\w.-]+%$/.test(v)) return true;                // %VAR% (windows)
  if (/^<[^>]+>$/.test(v)) return true;                  // <your-token>
  if (/\b(your|example|placeholder|changeme|change-me|replace|insert|todo|dummy|sample|redacted|none|null)\b/i.test(v)) return true;
  if (/^(?:x{3,}|\*{3,}|\.{3,}|-{3,}|_{3,})$/i.test(v)) return true;
  if (new Set(v.replace(/[-_]/g, "")).size <= 2) return true; // aaaa, 0000
  return false;
}

// An opaque token-shaped value — not a URL, path, or connection string (those
// are env-exposure's job), just an unbroken high-entropy blob.
function isOpaqueSecretValue(v) {
  if (typeof v !== "string" || v.length < 16) return false;
  if (looksLikePlaceholder(v)) return false;
  if (/[\s/:@\\]/.test(v)) return false;                 // exclude urls/paths/connstrings
  const hasLetter = /[A-Za-z]/.test(v), hasDigit = /[0-9]/.test(v);
  return (hasLetter && hasDigit) || /^[A-Fa-f0-9]{32,}$/.test(v) || /^[A-Za-z0-9+/_=-]{24,}$/.test(v);
}

export function analyzeHardcodedSecrets(entry) {
  const findings = [];
  const seen = new Set();
  const add = (secretType, severity, location) => {
    const key = `${location}::${secretType}`;
    if (seen.has(key)) return false;
    seen.add(key);
    findings.push({
      type: "hardcoded-secret", severity, secretType, location,
      description: `Hardcoded ${secretType} in ${location} (value redacted) — a literal credential in a config file leaks through dotfiles, backups, and version control. Reference it with \${ENV_VAR} or a secret manager instead`,
    });
    return true;
  };

  // Known provider token shapes anywhere in command/args.
  const scanText = (text, location) => {
    let hit = false;
    for (const { pattern, type, severity } of CONFIG_SECRET_PATTERNS) {
      if (pattern.test(text)) hit = add(type, severity, location) || hit;
    }
    return hit;
  };

  scanText(entry.command || "", "command");
  scanText(Array.isArray(entry.args) ? entry.args.join(" ") : "", "args");

  const env = entry.env || {};
  for (const [k, v] of Object.entries(env)) {
    if (typeof v !== "string") continue;
    const location = `env var "${k}"`;
    const knownFormat = scanText(v, location);
    // Fallback: a sensitive-named var holding a concrete opaque token. Skip if a
    // specific provider format already fired for this var (no double report).
    if (!knownFormat && SENSITIVE_ENV_PATTERNS.some(p => p.pattern.test(k)) && isOpaqueSecretValue(v)) {
      add("credential", "critical", location);
    }
  }
