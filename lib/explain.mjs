...
function classifyWithReason(name, desc = "") {
  ...
  const descChecks = [
    [/execut|command|shell|bash|sudo|spawn|fork|eval/, "critical", "description mentions code execution"],
    [/delet|remov|drop|destruct|truncat|wipe|purge/, "critical", "description mentions destructive operations"],
    [/read.*file|write.*file|filesystem|file.*system/, "high", "description mentions filesystem access"],
    [/credentials|secret|password|api.?key|token|auth/, "high", "description mentions credentials"],
    [/http|request|fetch|curl|wget|network/, "high", "description mentions network access"],
    [/database|query|sql|mongo|redis|dynamo/, "high", "description mentions database access"],
    [/email|message|notification|sms|slack/, "medium", "description mentions messaging"],
    [/browse|screenshot|puppeteer|playwright|selenium/, "medium", "description mentions browser automation"],
  ];
  for (const [re, risk, reason] of descChecks) {
    if (re.test(lowerDesc)) return { risk, reason, matched: re.toString() };
  }
