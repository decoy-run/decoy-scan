...
// ─── Hardcoded Credential Patterns (config command/args/env values) ───
//
// Distinctive provider token shapes. These are anchored to a vendor prefix +
// length, so a real value matches but a placeholder ("sk-xxx", "your-token")
// does not. Used to flag literal secrets pasted into a server config, where
// env-exposure only flags the *name* of a sensitive var.
export const CONFIG_SECRET_PATTERNS = [
  { pattern: /\bsk-ant-[A-Za-z0-9_-]{24,}/, type: "anthropic-key", severity: "critical" },
  { pattern: /\bsk-(?:proj-)?[A-Za-z0-9]{20,}/, type: "openai-key", severity: "critical" },
  { pattern: /\b(?:ghp|gho|ghu|ghs|ghr)_[A-Za-z0-9]{36,}/, type: "github-token", severity: "critical" },
  { pattern: /\bgithub_pat_[A-Za-z0-9_]{22,}/, type: "github-fine-grained-pat", severity: "critical" },
  { pattern: /\b(?:AKIA|ASIA)[A-Z0-9]{16}\b/, type: "aws-access-key", severity: "critical" },
  { pattern: /\bxox[baprs]-[A-Za-z0-9-]{10,}/, type: "slack-token", severity: "critical" },
  { pattern: /\b[rs]k_(?:live|test)_[A-Za-z0-9]{16,}/, type: "stripe-key", severity: "critical" },
  { pattern: /\bAIza[A-Za-z0-9_-]{35}\b/, type: "google-api-key", severity: "critical" },
  { pattern: /\bglpat-[A-Za-z0-9_-]{20,}/, type: "gitlab-pat", severity: "critical" },
  { pattern: /\bxai-[A-Za-z0-9]{20,}/, type: "xai-key", severity: "critical" },
  { pattern: /-----BEGIN (?:RSA |EC |OPENSSH |PGP |DSA |ENCRYPTED )?PRIVATE KEY/, type: "private-key", severity: "critical" },
  { pattern: /\bBearer\s+[A-Za-z0-9._~+/-]{24,}=*/, type: "bearer-token", severity: "high" },
];
