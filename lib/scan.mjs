...
import {
  classifyTool, detectPoisoning, analyzeServerCommand, analyzeHardcodedSecrets, analyzeEnvExposure,
...
export async function scan({ probe = true, advisories = true, skills = false, customPatterns = [], configs: preloadedConfigs } = {}) {
  ...
  for (let i = 0; i < serverOrder.length; i++) {
    ...
    for (const f of analyzeServerCommand(entry)) {
      const source = f.type === "potential-typosquat" ? "typosquat"
        : (f.type === "unpinned-remote-source" || f.type === "unpinned-dist-tag") ? "supply-chain"
        : "server-command";
      server.findings.push({ ...f, source });
      results.summary.suspicious++;
    }

    // Hardcoded credentials (literal secret values in command/args/env)
    for (const f of analyzeHardcodedSecrets(entry)) {
      server.findings.push({ ...f, source: "hardcoded-secret" });
      results.summary.suspicious++;
    }
