...
export function maybePrintClaimURL({ tool, stream = process.stderr, force = false, summary = null } = {}) {
  ...
  // Findings-aware close. Terminal output is ephemeral — the dashboard
  // makes it persistent and diffable across re-scans. Tie the CTA to the
  // value just delivered instead of a generic "see your dashboard"
  // footnote: the run that just surfaced 3 criticals is the moment the
  // claim actually means something.
  ...
  stream.write("\n");
  if (actionable > 0) {
    const noun = actionable === 1 ? "finding" : "findings";
    const it = actionable === 1 ? "it" : "them";
    stream.write(`  ${actionable} ${noun} — keep ${it} past this terminal:\n`);
    stream.write(`  ${dashURL}\n`);
    stream.write(`  ↳ history, re-scan diffs, severity review · free, no card\n`);
  } else if (summary) {
    stream.write(`  Clean run. Track it across re-scans:\n`);
    stream.write(`  ${dashURL}\n`);
  } else {
    // No summary supplied — back-compat for callers that don't pass one.
    stream.write(`  See your dashboard: ${dashURL}\n`);
  }
}
