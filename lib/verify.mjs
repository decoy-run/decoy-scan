...
// scan findings. Requires a free Decoy account (claim install_id at
// app.decoy.run/d/<installId>). Free: 5/account/month. Team: 30/seat/day.
// Business: 60/seat/day.
//
// Returns:
//   { ok: true, verified: [...], stats: {...}, quota: {...} | null }
//   { ok: false, status, code, message, claimUrl?, upgradeUrl?, quota?, cap? }
//
...
export async function verifyFindings({ results, token } = {}) {
  ...
  if (res.status === 401 && body.code === "no_account") {
    return {
      ok: false,
      status: 401,
      code: "no_account",
      message: body.error || "AI verification requires a free account",
      detail: body.detail,
      claimUrl: body.claim || `https://app.decoy.run/d/${installId}`,
    };
  }
  if (res.status === 402) {
    return {
